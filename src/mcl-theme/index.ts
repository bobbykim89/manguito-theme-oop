...
export class MclClass<T extends ClassType> {
  ...
  private getBorderWidthClass(): string {
    if (!this.validator.validateRangeType()) {
      return ' '
    }
    if (this.isBorderWidthClass(this.classValue)) {
  ...
  private getOpacityClass(): string {
    if (!this.validator.validateOpacityType()) {
      return ' '
    }
    if (this.isOpacityClass(this.classValue)) {
  ...
  private getMarginClass(): string {
    if (!this.validator.validateSpaceType()) {
      return ' '
    }
    if (this.isSpacingClass(this.classValue)) {
  ...
  private isBorderWidthClass(arg: InputOptions): arg is Range<1, 12> {
    return (typeGroup.borderWidthType as readonly string[]).includes(
      this.classType
    )
  }
  private isOpacityClass(arg: InputOptions): arg is OpacityRange {
    return (typeGroup.scaleType as readonly string[]).includes(this.classType)
  }
  private isSpacingClass(arg: InputOptions): arg is SpacingLevel {
